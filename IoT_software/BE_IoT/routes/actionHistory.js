...
// ========== 3. Regex pattern (định dạng tìm kiếm theo thời gian) ==========
...
router.get("/filter", async (req, res) => {
  try {
    ...
    let { page = 1, pageSize = 10, device, state, search } = req.query;
    ...
    // ----- 4.2 Xử lý lọc device, state -----
    if (device && device !== "all") query.device = device;
    if (state && state !== "all") query.state = state;

    // ----- 4.3 Xử lý search theo thời gian cụ thể -----
    ...
    console.log("ActionHistory query:", JSON.stringify(query));
